---
title: Revamping BubbleCheck
authors: taylorreece
tags: [bubblecheck]
description: BubbleCheck runs on a pretty old tech stack. Let's look at what currently exists (and what needs to change).
---

In 2013 I built [BubbleCheck](https://bubblecheck.reecemath.com/), an automated grading tool that lets teachers build multiple choice exams, scan in their students' completed exams, and review the results.
...
![XKCD on time savings](https://imgs.xkcd.com/comics/is_it_worth_the_time_2x.png)
...
I had built some web apps on top of the LAMP stack for my college's student government website, [Oleville](https://www.oleville.com/), and it's the stack I felt most comfortable with.
...
The processing of exams that teachers scan in is done in C++ using [MuPDF](https://mupdf.com/) to convert PDF scans to PNG images, and [OpenCV](https://opencv.org/) to process the images.
Exams that teachers upload are placed in a directory, and a CRON job that kicks off every minute grades the exams, one at a time.

The frontend is rendered server-side with Laravel and leverages the [Bootstrap](https://getbootstrap.com/) CSS framework.
The frontend communicates with the backend using [jQuery](https://jquery.com/).

The web services does not scale.
From 2013 until 2017 I hosted BubbleCheck on a shared [Bluehost](https://www.bluehost.com/) node with hosted [MySQL](https://www.mysql.com/).
Now, it's installed on a single EC2 in [AWS](https://aws.amazon.com/) that runs PHP, MySQL, and has a mounted [EFS](https://aws.amazon.com/efs/) filesystem for storing scanned exams.
...
In the upcoming blog posts, I'll write about my journey as I rebuild BubbleCheck using modern technologies, to make a maintainable, scalable web and mobile app, and I share my learnings along the way.
